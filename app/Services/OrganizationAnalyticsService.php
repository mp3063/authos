...
class OrganizationAnalyticsService extends BaseService implements OrganizationAnalyticsServiceInterface
{
    ...
    /**
     * Format organization response with user metrics
     */
    private function formatOrganizationResponse(Organization $organization, bool $detailed = false): array
    {
        $data = $this->getBaseOrganizationData($organization);

        if ($detailed) {
            $data = array_merge($data, $this->getDetailedOrganizationData($organization));
        }
     ...
     */
    private function getBaseOrganizationData(Organization $organization): array
    {
     ...
     */
    private function getDetailedOrganizationData(Organization $organization): array
    {
        $data = [
            'recent_activity' => [],  // Add placeholder for recent activity
        ];
    ...
    private function formatUserResponse(User $user, Organization $organization): array
    {
        ...
        return [
            ...
            'applications' => $userApplications->map(function ($app) {
                return [
                    'id' => $app->id,
                    'name' => $app->name,
                    'client_id' => $app->client_id,
                    ...
                    'granted_at' => $app->pivot->granted_at,
                ];
            }),
